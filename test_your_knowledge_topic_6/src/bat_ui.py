...
class BatUI():
    ...
    def get_current_screen(self):
        ...
        Retrieve the current menu screen.
            ...
            Returns:
                A string representation of the current menu screen. 
                Possible values are
                "MAIN MENU", "LOAN ITEM", "RETURN ITEM", 
                "SEARCH FOR PATRON", "REGISTER PATRON",
    ...
    def run_current_screen(self):
        ...
        Run the current menu screen.
          If necessary, transition to a new menu screen.
        '''
        self._current_screen = self._current_screen()

    def _main_menu(self):
        '''
        The main menu screen of BAT. Presents the user with 6 options 
        (loan item, returnitem, search for patron, register patron, 
        validate makerspace access, and quit),
    ...
    def _loan_item(self):
        ...
        - ask the user for the length of the loan in days 
        (from 1 - 365 inclusive)
        - loan the item if the specified user is 
        allowed to loan the specified item

        If the process fails at any point (e.g., an item with the given 
        ID can'tbe found in the catalogue, a patron with the given name 
        and age can't befound in the patron database), return to the
        main menu screen.

        If the loan is successfully processed, print a message then return 
        ...
        item = search.find_item_by_id(item_id,
                                       self._data_manager._catalogue_data)
        ...
        if item == None:
            ...
            if choice == 'y':
                print(
                    "Enter details of patron wanting to loan this item...")
                ...
                if patron == None:
                    ...
                    loan_success = logic.process_loan(
                        patron, item, length_of_loan)

                    if loan_success:
                        print(
                            f"Loan of {item._name} to {patron._name} successfully recorded")
                    else:
                        print(
                            f"Sorry, {patron._name} is not able to borrow {item._name}")
            else:
    ...
    def _return_item(self):
        ...
        - ask for the item ID of the item being returned, and continue 
        asking until a valid item ID is entered
        - process the return of the item

        If the process fails at any point (e.g., a patron with the given 
        name and agecan't be found in the patron database), return to the 
        main menu screen.

        If the return is successfully processed, print a message then return
        to the main menu screen.
        '''
        print("""
            --------------------
        ...
        if patron == None:
            ...
            choice = user_input.read_integer(
                "Enter the ID of the item to return: ")
            while choice not in loaned_ids:
                print(
                    f"That is not an ID of an item currently loaned by {
                        patron._name}")
                choice = user_input.read_integer(
                    "Enter the ID of the item to return: ")
    ...
    def _search_for_patron(self):
        '''
        The patron search menu screen of BAT. Allows users to enter a 
        name or age (one or the other, not both), and have the details of any
        patrons with that name or age printed out.
        '''
        print("""
            --------------------
            | BAT: Find Patron |
        ...
        match choice:
            case 1:
                ...
                patrons_found = search.find_patron_by_name(
                    name, self._data_manager._patron_data)
            case 2:
                age = user_input.read_integer("Enter age: ")
                patrons_found = search.find_patron_by_age(
                    age, self._data_manager._patron_data)
            case 3:
                return self._main_menu
            case _:
                return self._search_for_patron
    ...
    def _register_patron(self):
        '''
        The register patron menu screen of BAT. Allows the user to 
        create a new patron for the database. The user is asked for 
        a name, and an age (from 0 to 100 inclusive). The new patron
        will start with no outstanding fees, no loans,
    ...
    def _access_makerspace(self):
        ...
        if (patron == None):
            ...
            allowed = logic.can_use_makerspace(
                patron._age, 
